""" Code was taken from https://github.com/PRBonn/SHINE_mapping. """
...
def parallel_get_dict_values(dictionary, key_list, default=None, processes=None):
    ...
    with multiprocessing.Pool(processes=processes) as pool:
        value_list = pool.starmap(
            get_dict_values, [(dictionary, keys, default) for keys in chunks]
        )
    return [v for values in value_list for v in values]


class FeatureOctree(nn.Module):
    def __init__(self, octree_config, scale, device="cuda"):
        super().__init__()

        # [0 1 2 3 ... max_level-1 max_level], 0 level is the root, which have 8 corners.
        self.max_level = octree_config.tree_level_world
        # the number of levels with feature (begin from bottom)
        self.leaf_vox_size = octree_config.leaf_vox_size
        self.featured_level_num = octree_config.tree_level_feat
        self.free_level_num = self.max_level - self.featured_level_num + 1
        self.feature_dim = octree_config.feature_dim
        self.feature_std = octree_config.feature_std
        self.polynomial_interpolation = octree_config.poly_int_on
        self.device = device
        self.scale = scale
    ...
    def get_morton(self, sample_points, level):
        points = kal.ops.spc.quantize_points(
            sample_points, level
        )  # quantize to interger coords
        points_morton = kal.ops.spc.points_to_morton(points)  # to 1d morton code
        sample_points_with_morton = torch.hstack(
            (sample_points, points_morton.view(-1, 1))
        )
    ...
    def get_octree_nodes(self, level):  # top-down
        ...
        nodes_coord_scaled = (
            (nodes_spc_np * node_size) - 1.0 + 0.5 * node_size
        )  # in the -1 to 1 kaolin space
    ...
    def update(self, surface_points, incremental_on=False):
        ...
        for i in range(self.max_level + 1):  # for each level (top-down)
            ...
            corners_morton = (
                kal.ops.spc.points_to_morton(corners_unique).cpu().numpy().tolist()
            )
            if len(self.corners_lookup_tables[i]) == 0:  # for the first frame
                ...
                fts = self.feature_std * torch.randn(
                    len(corners_dict) + 1, self.feature_dim, device=self.device
                )
                ...
                for m in corners_morton:
                    if m not in self.corners_lookup_tables[i]:  # add new keys
                        self.corners_lookup_tables[i][m] = len(
                            self.corners_lookup_tables[i]
                        )
                new_feature_num = len(self.corners_lookup_tables[i]) - pre_size
                new_fts = self.feature_std * torch.randn(
                    new_feature_num + 1, self.feature_dim, device=self.device
                )
                new_fts[-1] = torch.zeros(1, self.feature_dim)
                cur_featured_level = i - self.free_level_num
                self.hier_features[cur_featured_level] = nn.Parameter(
                    torch.cat((self.hier_features[cur_featured_level][:-1], new_fts), 0)
                )
                if incremental_on:
                    new_weights = torch.zeros(
                        new_feature_num + 1, self.feature_dim, device=self.device
                    )
                    self.importance_weight[cur_featured_level] = torch.cat(
                        (self.importance_weight[cur_featured_level][:-1], new_weights),
                        0,
                    )
                    self.features_last_frame[cur_featured_level] = self.hier_features[
                        cur_featured_level
                    ].clone()

            corners_m = kal.ops.spc.points_to_morton(corners).cpu().numpy().tolist()
            indexes = (
                torch.tensor([self.corners_lookup_tables[i][x] for x in corners_m])
                .reshape(-1, 8)
                .numpy()
                .tolist()
            )
            new_nodes_morton = (
                kal.ops.spc.points_to_morton(new_nodes).cpu().numpy().tolist()
            )
    ...
    def get_indices(self, coord):
        self.hierarchical_indices = (
            []
        )  # initialize the hierarchical indices list for the batch points x
        for i in range(self.featured_level_num):  # bottom-up, for each level
            ...
            # convert to 1d morton code for the voxel center
            points_morton = kal.ops.spc.points_to_morton(points).cpu().numpy().tolist()
            # if not in the look up table, then assign all -1
            ...
            indices_list = [
                self.nodes_lookup_tables[current_level].get(p, features_last_row)
                for p in points_morton
            ]
    ...
    def query_feature_with_indices(self, coord, hierarchical_indices):
        ...
        for i in range(self.featured_level_num):  # for each level
            ...
            coeffs = self.interpolat(
                coord, current_level, self.polynomial_interpolation
            )
            sum_features += (
                self.hier_features[feature_level][hierarchical_indices[i]] * coeffs
            ).sum(1)
    ...
    def query_feature(self, _coord, faster=False):
        self.set_zero()  # set the trashbin feature vector back to 0 after the feature update
        coord = _coord * self.scale
        if faster:
            # it would only be faster when the input coords have a lot share the same morton code
    ...
    def cal_regularization(self):
        ...
        for i in range(self.featured_level_num):  # for each level
            ...
            difference = (
                self.hier_features[feature_level][unique_indices]
                - self.features_last_frame[feature_level][unique_indices]
            )
            # regularization for continous learning weighted by the feature importance and the change magnitude
            regularization += (
                self.importance_weight[feature_level][unique_indices] * (difference**2)
            ).sum()
    ...
    def get_indices_fast(self, coord):
        ...
        for i in range(self.featured_level_num):  # bottom-up
            ...
            points_morton = (
                kal.ops.spc.points_to_morton(points).cpu().numpy().tolist()
            )  # convert to 1d morton code for the voxel center
            features_last_row = [
                -1 for t in range(8)
            ]  # if not in the look up table, then assign -1

            dups_in_mortons = dict(
                self.list_duplicates(points_morton)
            )  # list the x with the same morton code (samples inside the same voxel)
            ...
            for p in dups_in_mortons.keys():
                ...
                corner_indices = self.nodes_lookup_tables[current_level].get(
                    p, features_last_row
                )
    ...
    def print_detail(self):
        ...
        for level in range(self.featured_level_num):
            level_vox_size = self.leaf_vox_size * (
                2 ** (self.featured_level_num - 1 - level)
            )
        ...
        total_map_memory = (
            total_vox_count * self.feature_dim * 4 / 1024 / 1024
        )  # unit: MB
        print(
            "memory: %d x %d x 4 = %.3f MB"
            % (total_vox_count, self.feature_dim, total_map_memory)
        )
        print("--------------------------------")
